...
const PlusOneDietaryInformation: React.FC<DietaryInformationProps> = ({
  register,
  errors,
  inputClass,
  control,
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {fields.map((field, index) => (
          <div key={field.id} className="flex items-center">
            <select
              id={`plus_one_dietary_restrictions_${index}`}
              className={inputClass}
              {...register(`plus_one.dietary_restrictions.${index}.value`)}
            >
...
export default PlusOneDietaryInformation;
